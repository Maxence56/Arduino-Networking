...
//a_p c'est le pointeur vers le contenu du packet
void ArdNetworkLayerTwoAck::sendRequest(PktBufPtr a_p, L2Addr a_l2_dst_addr)
{
  ...
  // Before adding a packet to the transmission queue, we need to make sure
  // that it is not full.  If it is, we print a message and drop the packet.
  bool res = false; //res : prise en compte du message par le layer2
  if (m_queue.isFull())
  {
    info_pr(ARD_F("L2: dropping a frame because the tx queue is full\n"));
    res = false;
  ...
  if (m_upper_layer) //pointeur initialisé
  {
    m_upper_layer->dataHandlingDone(ard_move(a_p), res);
  }
}
...
void ArdNetworkLayerTwoAck::checkQueue()
{
  if (!m_sending && !m_queue.isEmpty())
  {
    ...
    PktBufPtr l2_p = m_queue.removeElement();
    // Store the last sequence number sent
    //on update le last seq num sent
...
{ //pointeur layer 1 
  ...
  L2Addr dst_addr = L2Message::getDstAddr(l2_p->data);
  // le 3eme octet correspond au type du message 
  if (l2_p->data[2] != L2_TYP_ACK) //data 
  {
    if (m_tx_count < 3) //nbre de tentatives
    {
      m_timer_handler.startTimer(1000, &m_timer_func);
      // functor rappel dosend
    }
    else
    {
      // we have transmitted this packet three times, we must reset the counter
  ...
  m_tx_count++;
  m_send_interface->sendRequest(ard_move(l2_p), dst_addr); //couche1 transmission
}
...
void ArdNetworkLayerTwoAck::dataHandlingDone(PktBufPtr l2_p, bool res)
{
  if (m_tx_count < 3)
  {
    m_timer_func.setPktBuff(ard_move(l2_p)); //timerfunc gere l2_p pour la transmission 
  }
  else
  {
    m_tx_count = 0;
...
{
  debug_pr(ARD_F("Layer 2 onDataReceived, size: "), int(a_p->curr_size),
           ARD_F("\n")); // quand on recoit de la couche inf
  if (a_p->curr_size < 4)
  {
    info_pr(ARD_F("Layer 2 ignoring data frame, too small, size: "),
            int(a_p->curr_size), ARD_F("\n"));
  ...
  if (l2_msg.m_msg_type == L2_TYP_ACK && m_contexts.retrieveContextByAdd(l2_msg.m_src_add.m_addr)->m_last_seq_num_sent == l2_msg.m_seq_num)
  { // ack et dernier num seq envoyé à cette adresse
    m_timer_handler.stopTimer(); // on sait que c le bon ack
    m_tx_count = 0;
  }
  ...
  m_upper_layer->onDataReceived(ard_move(payload_p), AnyAddr(l2_msg.m_src_add),
                                m_l_id); //on remonte le ack au Lay3
  if (m_contexts.isAddInPool(l2_msg.m_dst_add.m_addr) == false)
  {
    m_contexts.getNewContextForAdd(l2_msg.m_dst_add.m_addr);
  } //? a verfier

  if (l2_msg.m_msg_type != L2_TYP_ACK)
  {
    L2Message ack_msg(m_this_addr, l2_msg.m_src_add, 0, m_contexts.retrieveContextByAdd(l2_msg.m_dst_add.m_addr)->m_send_seq_num, L2_TYP_ACK);
    ArdUniquePtrMemPool<ArdPktBuffer> a_p1 = ack_msg.serialize(m_mem_pool); //on genere un ack
    if (!(m_send_interface))
    {
      ard_error(ARD_F("Layer 2 sendRequest: lower layer is not set\n "));
    }
    L2Addr dst_addr = l2_msg.m_src_add;
    m_send_interface->sendRequest(ard_move(a_p1), dst_addr); //envoie du message
  }
}
